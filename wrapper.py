...
def merge_fields_below_percentage( merge_below_percentage, size_successful_queries, datapoints, storage,
    ...
    # we precreated a bigger size, now be delete the not needed part
    ...
    merge_below_abs = int( merge_below_percentage / 100 * total_power_last_block )

    merged_elements = [0] * size_successful_queries
    to_delete = []
    for id, power in storage.items():
...
def get_account_power():
    ...
    type      = request.args.get( "type", "total" ) # or proxy
    # TODO insert checks for udd
...
@app.route('/get_voteable_votes')
def get_voteable_votes(): # returns the voting power of a worker over time with each account voted for him
    print( "REQUEST RECEIVED" )
    global voteable_votes_cache

    from_date = request.args.get( "from", "2019-01-01T00:00:00" ) # past date
    to_date   = request.args.get( "to", "2019-07-15T00:00:00" )   # date after past date
    vote_id   = request.args.get( "vote_id", "2:614" ) # == decentralized worker
    datapoints = int( request.args.get( "datapoints", 100 ) )
